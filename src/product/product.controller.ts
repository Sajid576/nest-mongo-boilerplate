...
export class ProductController {
  ...
  @ApiOperation({ summary: 'Get products by query' })
  @ApiResponse({
    status: 200,
    description: 'Return success code and found products result[Array]',
  })
  @ApiResponse({
    status: 403,
    description:
      'Forbidden. Return error code and description in founding products',
  })
  @Get('/')
  ...
  async findByCustomer() {
    const result = await this.productService.findByCustomer();
    if (!result.success) {
      return new ResponseError(result.data, 'Fail').getResponse();
    }
    return new ResponseSuccess(result.data, 'Success').getResponse();
